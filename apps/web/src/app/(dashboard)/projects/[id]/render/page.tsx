...
export default function ExportResultPage() {
  ...
  const [musicSuggestion, setMusicSuggestion] = useState<any>(null);
  ...
  const firstMangaPath = project?.mangaPages?.[0]?.url || project?.image || "";

  const handleSuggestMusic = async () => {
    if (!firstMangaPath) return;
    const result = await suggestMusic(firstMangaPath);
  ...
  const handleStartRender = async () => {
    ...
    await submitRender(firstMangaPath, project?.audioInfo?.url || "");
  };

  return (
    <main className="max-w-360 mx-auto px-4 sm:px-6 lg:px-8 py-16 flex flex-col gap-12 relative overflow-hidden font-display">
      {/* Background Pattern */}
      <div
      ...
      <div className="z-10 grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div className="relative group bg-surface-dark border-6 border-black p-4 shadow-neo-orange">
            ...
            >
              ...
              <div className="absolute bottom-6 left-6 font-mono text-primary text-[10px] bg-black/80 px-4 py-3 border-2 border-primary/20 backdrop-blur-sm space-y-1 uppercase tracking-widest leading-relaxed italic font-bold">
                ...
                <p>&gt; LAYER: {currentJob?.currentStage || "FX_OVERLAY_03"}</p>
                <p>&gt; SHADER: {currentJob?.status?.toUpperCase() || "NEON_VULKAN_PIPE"}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-white text-black border-6 border-black p-6 shadow-hard transition-all hover:bg-primary group">
              ...
              <p className="text-3xl font-black italic tracking-tighter">
                {project?.settings?.resolution || "4K_ULTRA_HD"}
              </p>
            </div>
            <div className="bg-white text-black border-6 border-black p-6 shadow-hard transition-all hover:bg-secondary group">
              ...
              <p className="text-3xl font-black italic tracking-tighter">
                {project?.audioInfo?.bpm ? `${project.audioInfo.bpm}_BPM` : "320_KBPS"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-10">
          {/* Success Stamp Area */}
          <div className="flex-1 bg-surface-dark border-6 border-black border-dashed flex items-center justify-center p-12 relative overflow-hidden shadow-hard hover:shadow-neo hover:border-solid transition-all group min-h-100">
            ...
            <div className="relative w-72 h-72 flex items-center justify-center">
              {/* Rotating Ring */}
              <div className="absolute inset-0 border-24 border-black border-t-primary rounded-full animate-spin transition-transform duration-4000"></div>
              <div className="absolute inset-4 border-12 border-dashed border-white/5 rounded-full"></div>
          ...
          <div className="bg-white text-black border-6 border-black p-8 shadow-hard space-y-8">
            <div className="flex items-center justify-between border-b-6 border-black pb-4">
              ...
              <Link
                ...
                className="flex items-center gap-2 bg-black text-white px-4 py-2 font-black uppercase text-[10px] border-2 border-black hover:bg-primary hover:text-black transition-all italic shadow-hard-xs hover:translate-x-0.5 hover:translate-y-0.5"
              >
            ...
            <div className="flex flex-wrap gap-4">
              <button
                ...
                className="flex-1 min-w-45 bg-black text-white px-4 py-4 font-black uppercase text-xs border-2 border-black hover:bg-primary hover:text-black transition-all italic shadow-hard-xs hover:translate-x-0.5 hover:translate-y-0.5 disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {currentJob?.status === "queued"
                  ? "Render Queued"
                  : currentJob?.status === "processing"
              ...
              <button
                ...
                className="flex-1 min-w-45 bg-transparent text-black px-4 py-4 font-black uppercase text-xs border-2 border-black hover:bg-black hover:text-white transition-all italic shadow-hard-xs hover:translate-x-0.5 hover:translate-y-0.5"
              >
                Cancel Render
              </button>
            </div>

            <div className="border-4 border-dashed border-black p-5 bg-black/5 space-y-3">
              <div className="flex items-center justify-between gap-4">
                <h4 className="font-black uppercase italic tracking-tighter">ML Music Suggestion</h4>
                <button
                  onClick={handleSuggestMusic}
                  disabled={!firstMangaPath || isLoadingProject}
                  className="bg-primary text-black px-4 py-2 font-black uppercase text-[10px] border-2 border-black disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Suggest Music
                </button>
              </div>
              <p className="font-mono text-xs text-black/60">
                {firstMangaPath ? firstMangaPath : "Upload a manga chapter to enable ML suggestions."}
              </p>
              {!project?.audioInfo?.url && (
                <p className="font-mono text-xs text-black/60">
                  Upload an audio track before starting a render.
                </p>
              )}
              {musicSuggestion && (
                <pre className="overflow-auto bg-black text-primary p-4 border-2 border-black text-[10px] leading-relaxed">
                  {JSON.stringify(musicSuggestion, null, 2)}
                </pre>
              )}
            </div>
            ...
            {currentJob?.status === 'completed' && currentJob.resultUrl ? (
              <a
                ...
                download
                className="w-full bg-primary hover:bg-black hover:text-primary border-6 border-black py-6 px-10 flex items-center justify-between shadow-hard hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all group italic"
              >
                <div className="flex flex-col items-start gap-1">
                  ...
                  <span className="font-mono text-[10px] font-black opacity-60 uppercase tracking-[0.2em]">
                    {currentJob?.status?.toUpperCase() || "MP4"} • {currentJob?.fileSizeBytes ? `${Math.round(currentJob.fileSizeBytes/1024/1024)}MB` : '—'} • HDR_SYNTH
                  </span>
                </div>
              ...
              <button className="w-full bg-primary hover:bg-black hover:text-primary border-6 border-black py-6 px-10 flex items-center justify-between shadow-hard hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all group italic disabled:opacity-40" disabled>
                <div className="flex flex-col items-start gap-1">
                  <span className="font-black text-2xl uppercase italic tracking-tighter">
                    Download Master
                  </span>
                  <span className="font-mono text-[10px] font-black opacity-60 uppercase tracking-[0.2em]">
                    {currentJob?.status?.toUpperCase() || "MP4"} • 248MB • HDR_SYNTH
                  </span>
                </div>
                <Download className="w-12 h-12 bg-black text-primary p-2 border-4 border-primary group-hover:bg-primary group-hover:text-black group-hover:border-black transition-all" />
              </button>
      ...
      <footer className="w-full border-t-6 border-black dark:border-white py-12 flex flex-col md:flex-row justify-between items-center gap-8 z-10 bg-background-dark/80 backdrop-blur-md px-10 -mx-10 mt-12 bg-manga-dots bg-size-[24px_24px] opacity-80">
        <div className="flex gap-12 font-mono text-xs font-black uppercase tracking-widest italic text-white/40">
          <span className="flex items-center gap-3">
            ...
            RENDER_ENGINE: <span className="text-white">{currentJob?.status || "VULKAN_GPU_RTX"}</span>
          </span>
          <span className="flex items-center gap-3">
            QUEUE_POS: <span className="text-primary">{currentJob ? "#1_STABLE" : "#IDLE"}</span>
          </span>
        </div>
        ...
        <span className="font-mono text-[10px] text-white/20 italic tracking-[0.4em] uppercase">
          {isLoadingProject ? "SYNCING_PROJECT_DATA" : "Build_2.4.0_Stable_Sync"}
        </span>
      </footer>
    </main>
  );
...
function SocialButton({
  ...
  return (
    <button
      className="relative group border-6 border-black p-6 flex flex-col items-center justify-center gap-4 shadow-hard hover:shadow-neo hover:translate-x-1 hover:translate-y-1 transition-all h-40 overflow-hidden"
